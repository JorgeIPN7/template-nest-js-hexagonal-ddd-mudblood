...
- **Mocking by layer:** no mocks in `domain/`; hand-written port fakes in `application/` (see `__tests__/helpers/in-memory-user.repository.ts`), never `jest.mock`; repositories are tested against real PostgreSQL in the E2E suite. Modules, TypeORM repositories, `data-source.ts` and migrations are excluded from _unit_ coverage on purpose — `test/jest-e2e.config.mjs` measures exactly those files with its own threshold, so the exclusion is verified rather than assumed.
